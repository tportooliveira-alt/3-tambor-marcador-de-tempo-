...
private fun PreviewWithRoi(vm: PhotocellViewModel, displayRotation: Int) {
    ...
    val geometry = remember(viewSize, cap, displayRotation) {
        PreviewGeometry(viewSize.first.toFloat(), viewSize.second.toFloat(),
            ...
            cameraRendersNatural = cap?.mode?.highSpeed == false)
    }
    val s = vm.settings
    // ROI mapeada para o buffer sempre que a geometria ou a linha mudam
    LaunchedEffect(geometry, s.lineXFraction, s.bandTopFraction, s.bandBottomFraction, s.stripWidthPx) {
        if (viewSize.first > 0) {
                ...
                TextureView(ctx).apply {
                    surfaceTextureListener = object : TextureView.SurfaceTextureListener {
                        override fun onSurfaceTextureAvailable(st: SurfaceTexture, w: Int, h: Int) {
                            cap?.let { st.setDefaultBufferSize(it.size.width, it.size.height) }
                            vm.openCamera(Surface(st), w, h)
                        }
...
private fun RoiOverlay(vm: PhotocellViewModel, geometry: PreviewGeometry) {
    ...
    Canvas(
        Modifier.fillMaxSize().pointerInput(locked, s) {
...
private fun SidePanel(vm: PhotocellViewModel, onHistory: () -> Unit, onSettings: () -> Unit) {
    ...
    Column(
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            Button(onClick = { vm.arm() }, enabled = vm.canArm,
...
private fun DiagnosticsBlock(vm: PhotocellViewModel) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(3.dp)) {
        ...
        Text(cap?.let { "${it.size.width}x${it.size.height} · ${it.mode.label} (medido %.1f FPS)".format(d.measuredFps) } ?: "Sondando câmera…", style = mono)
        Text(if (ls.locked) "Exposição 1/%.0f s · ISO %d · travada".format(1e9 / maxOf(ls.exposureNs, 1L), ls.iso)
        ...
        Text("Drops %d · custo/quadro %.0f µs · %s".format(snap.drops, d.lastFrameCostMicros, cap?.precisionText ?: ""), style = mono)
