...
class Scene(
    ...
    fun frameBytes(tFrame: Long): ByteArray {
        ...
        for (row in 0 until roi.height) {
            ...
            val flick = 1.0 + flickerAmp * sin(2.0 * PI * 120.0 * (tRow / 1e9))
            for (x in 0 until planeWidth) {
                val base = bgLevel + ((x * 7 + g * 3) % 11)
                var frac = 0.0
                if (g in rowsA..rowsB) {
                    val tx = edgeTimeAt(x)
                    frac = ((tRow + exposureNs - tx) / exposureNs).coerceIn(0.0, 1.0)
                }
                val value = (base + (objLevel - base) * frac) * flick + rng.gauss(noiseSigma)
                buf[row * stride + x] = value.roundToInt().coerceIn(0, 255).toByte()
            }
        }
        return buf
    }
...
object SimulationHarness {
    ...
    fun runCrossing(
        ...
        val cfg = PhotocellConfig(calibrationSamples = 32, calibrationMinSamplesForOutlier = 8,
            skewNs = if (knownSkew) skewNs else null, exposureNs = exposureNs)
        ...
        return if (st == null) SimulationResult(false, 0, 0, 0, 0, eng.lag, eng.threshold ?: 0.0, eng.state)
            ...
            eng.lag, eng.threshold ?: 0.0, eng.state)
    }
}
